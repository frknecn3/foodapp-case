...
<Tab.Screen
  ...
  options={{
    tabBarLabel: () => null,
    tabBarIconStyle:{flex:1,flexDirection:'column'},
    unmountOnBlur: true,
    tabBarIcon: ({focused}) => {
      return (
        <View style={{ alignItems: 'center', justifyContent: 'center' }}>
          {focused ? <HomeSvgActive /> : <HomeSvg />}
          <Text
            style={{
              color: focused ? '#66AE7B' : '#333333',
              fontWeight: focused ? '500' : '300',
              fontSize: moderateScale(12),
              marginTop: 5, // Adjust this for spacing between icon and text
            }}>
            Anasayfa
          </Text>
        </View>
    ...
    tabBarIcon: ({ focused }) => {
      return (
        <View style={{ 
          ...
          {focused ? <FavsActiveSvg /> : <FavsSvg />}
          <Text
            style={{
              color: focused ? '#66AE7B' : '#333333',
              fontWeight: focused ? '500' : '300',
              fontSize: moderateScale(12),
              marginTop: 5, // Adjust this as needed
            }}>
            Favorilerim
          </Text>
        </View>
...
<Tab.Screen
  ...
  options={{
    ...
    tabBarIcon: ({focused}) => {
      return (
        <View style={{ alignItems: 'center', justifyContent: 'center' }}>
          {focused ? <DiscoverActiveSvg /> : <DiscoverSvg />}
          <Text
            style={{
              color: focused ? '#66AE7B' : '#333333',
              fontWeight: focused ? '500' : '300',
...
<Tab.Screen
  ...
  options={{
    ...
    tabBarIcon: ({focused}) => {
      return (
        <View style={{ alignItems: 'center', justifyContent: 'center' }}>
          {focused ? <BasketActiveSvg /> : <BasketSvg />}
          <Text
            style={{
              color: focused ? '#66AE7B' : '#333333',
              fontWeight: focused ? '500' : '300',
